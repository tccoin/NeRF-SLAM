# from https://discuss.pytorch.org/t/how-to-debug-causes-of-gpu-memory-leaks/6741/18
# usage:
# import sys
# from gpu_profile import trace_calls
# os.environ['GPU_DEBUG'] = True
...
EMPTY_CACHE = True
gpu_profile_fn = f"log/gpu_mem_prof.txt"
# gpu_profile_fn = f"log/{datetime.datetime.now():%d-%b-%y-%H:%M:%S}-gpu_mem_prof.txt"
if 'GPU_DEBUG' in os.environ:
    print('profiling gpu usage to ', gpu_profile_fn)

open(gpu_profile_fn, 'w').close()  # overwrite old gpu_profile
...
def trace_calls(frame, event, arg):
    ...
    co = frame.f_code
    func_name = co.co_name
    # with open('log/gpu_profile.txt', 'a+') as f:
    #     f.write(f"{func_name}\n")
    ...
    try:
        ...
        exit()
    # with open('log/gpu_profile.txt', 'a+') as f:
    #     f.write(f"{func_name}, {trace_into.split(' ')}, {func_name in trace_into.split(' ')}\n")
